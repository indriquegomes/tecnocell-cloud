...
(async () => {
  ...
  const status = await page.locator('text=Aberto, text=Fechado').first().textContent().catch(() => null);
  console.log('Status:', status);

  // Se fechado, abre primeiro
  const abrirBtn = page.locator('button:has-text("Abrir Caixa")');
  if (await abrirBtn.isVisible().catch(() => false)) {
    await page.fill('input[name="valor_abertura"]', '20000');
    await abrirBtn.click();
    await page.waitForURL('**/operacao**', { timeout: 8000 });
