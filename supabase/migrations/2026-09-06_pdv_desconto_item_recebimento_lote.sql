...
create table if not exists public.recebimentos_lote_operacoes (
  id uuid primary key, pedido jsonb not null, resultado jsonb,
  created_at timestamptz not null default now()
);
alter table public.recebimentos_lote_operacoes enable row level security;
revoke all on public.recebimentos_lote_operacoes from public, anon, authenticated;
grant all on public.recebimentos_lote_operacoes to service_role;
...
begin
  ...
  for v_aloc in select item from jsonb_array_elements(p_alocacoes) item order by item->>'id' loop
    ...
    if v_lanc.loja_id is not null and v_lanc.loja_id is distinct from p_loja_id then raise exception 'Dívida pertence a outra loja.'; end if;
