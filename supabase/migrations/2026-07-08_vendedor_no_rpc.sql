-- ============================================================
-- Vendedor dentro do RPC (conselho, último baixo).
--
-- Antes: finalizar_venda criava a venda sem vendedor e o app fazia um
-- `update vendas set vendedor_*` DEPOIS do RPC — se esse update falhasse
-- (rede/timeout), a venda ficava sem vendedor (rastreabilidade perdida).
--
-- Agora: p_vendedor_id / p_vendedor_nome entram no insert da venda, dentro
-- da mesma transação. Sem janela de falha.
--
-- Retrocompatível (novos params default null). Idempotente (drop + create).
-- ============================================================

drop function if exists public.finalizar_venda(jsonb, jsonb, text, numeric, text, text, jsonb);
