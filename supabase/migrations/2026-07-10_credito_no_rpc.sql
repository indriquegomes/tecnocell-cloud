-- ============================================================
-- Crédito do cliente dentro do RPC (conselho, launch-blocker).
--
-- Antes: o crédito abatido na venda era debitado por uma chamada `usarCreditoVenda`
-- DEPOIS do RPC, fora da transação e com o erro engolido (.catch). Se falhasse
-- (rede/timeout), a venda fechava mas o saldo do cliente não caía → crédito grátis.
--
-- Agora: finalizar_venda recebe p_credito_valor. Se > 0, valida o saldo e insere
-- o lançamento de 'uso' na MESMA transação. Falha → a venda inteira reverte.
--
-- Retrocompatível (p_credito_valor default 0). Idempotente (drop + create).
-- ============================================================

drop function if exists public.finalizar_venda(jsonb, jsonb, text, numeric, text, text, jsonb, uuid, text);
...
begin
  ...
  if p_credito_valor > 0 and p_pessoa_id is not null then
    select coalesce(sum(case when tipo = 'uso' then -valor else valor end), 0) into v_saldo_credito
