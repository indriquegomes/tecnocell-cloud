-- finalizar_venda usava current_date, que le a data em UTC (fuso do banco),
-- nao em horario de Sao Paulo. Toda venda entre ~21h e meia-noite (SP)
-- gravava data_competencia/data_vencimento/data_pagamento com a data de
-- AMANHA. Mesma armadilha de fuso que o CLAUDE.md ja documenta, so que
-- dentro do RPC em vez do codigo do app. Unica mudanca: a linha
-- "v_today := current_date;" virou "v_today := (now() at time zone
-- 'America/Sao_Paulo')::date;" — o resto do corpo e identico ao que ja
-- estava no banco (conferido com pg_get_functiondef antes de escrever isto).
CREATE OR REPLACE FUNCTION public.finalizar_venda(p_itens jsonb, p_pagamentos jsonb, p_pessoa_id text, p_desconto numeric, p_observacoes text, p_deposito_id text, p_series jsonb DEFAULT '[]'::jsonb, p_vendedor_id uuid DEFAULT NULL::uuid, p_vendedor_nome text DEFAULT NULL::text, p_credito_valor numeric DEFAULT 0)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
