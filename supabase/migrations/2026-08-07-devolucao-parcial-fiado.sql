-- ============================================================
-- Devolução PARCIAL e ciente do pagamento (bug relatado pela Isa/Vitor).
--
-- Antes: ao devolver item de uma venda com fiado, o RPC CANCELAVA o
-- lançamento de fiado INTEIRO — não importava quanto foi devolvido nem
-- quanto era fiado. Resultado:
--   • devolver 1 de 10 itens zerava a dívida toda;
--   • venda mista (R$5 pix + R$5 fiado) virava bagunça — parecia que devia
--     a nota inteira.
--
-- Agora: o valor devolvido ABATE a dívida primeiro (só o que devolveu, não
-- o lançamento todo). O que sobrar foi pago pelo cliente → vira REEMBOLSO
-- conforme p_tipo_credito (dinheiro/pix/crédito na conta) ou nada
-- (sem_reembolso / cancelamento_fiado).
--
--   valor_devolvido  = soma dos itens devolvidos
--   abate_fiado      = min(valor_devolvido, dívida em aberto da venda)
--   reembolso        = valor_devolvido - abate_fiado
--
-- p_lancamento_pendente deixou de ser usado (o RPC calcula a dívida real
-- direto da tabela). Assinatura mantida p/ compatibilidade.
-- Idempotente. Só muda o bloco financeiro; estoque/série intactos.
-- ============================================================
...
declare
  ...
  v_fiado_lanc_id  text;   -- lancamentos.id é TEXT (tabela do SIGE)
...
begin
  ...
  -- retorno ao estoque — itens NÃO serializados (em bloco)
  for v_item in select * from jsonb_array_elements(p_itens) loop
  ...
  -- 1) abate o fiado SÓ pelo valor devolvido (não cancela tudo)
  ...
  -- 2) a parte paga vira reembolso conforme o modo escolhido
