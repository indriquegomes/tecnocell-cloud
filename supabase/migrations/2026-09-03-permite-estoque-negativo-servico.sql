-- Permite estoque NEGATIVO para produtos tipo serviço (impressão, taxa,
-- frete, recuperação, mão de obra). A loja vende serviço acima do estoque; a
-- trava "não vender mais que tem" recusava venda composta com serviço.
-- Solução: flag permite_estoque_negativo no produto + a trava dos RPCs pula
-- esses produtos (venda e movimentação manual).

alter table produtos add column if not exists permite_estoque_negativo boolean default false;

update produtos set permite_estoque_negativo = true
where nome ilike '%servi%' or nome ilike '%impress%' or nome ilike '%taxa%'
   or nome ilike '%frete%' or nome ilike '%recupera%' or nome ilike '%mão de obra%'
   or nome ilike '%mao de obra%' or nome ilike '%instala%' or nome ilike '%montagem%';

-- ── movimentar_estoque (saída/perda) ───────────────────────────────────────
