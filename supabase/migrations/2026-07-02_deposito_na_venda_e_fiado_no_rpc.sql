-- ============================================================
-- APLICADO EM PRODUÇÃO em 2026-07-01 (SQL Editor do Supabase) — pendências #2 e #3 do conselho
-- Idempotente (add column if not exists / create or replace) — seguro reaplicar.
--
-- #2  vendas passa a gravar deposito_id  -> devolução volta ao depósito certo
-- #3  os lançamentos (pago e fiado) são criados JÁ com venda_id dentro do RPC
--     -> elimina o link posterior no app (o await) e o race de vendas fiado
--        simultâneas. O lançamento nasce vinculado à venda.
-- ============================================================

-- #2 — coluna deposito_id na venda (nullable, FK; vendas antigas ficam null)
alter table vendas add column if not exists deposito_id text references depositos(id);

-- #2 + #3 — finalizar_venda: grava deposito_id na venda e venda_id nos lançamentos.
-- (Corpo idêntico ao de produção, com 3 linhas alteradas — marcadas com "-- ALTERADO".)
...
begin
  ...
  -- Inserir venda
  insert into vendas (numero, total, desconto, forma_pagamento_id, pessoa_id, observacoes, status, deposito_id) -- ALTERADO: + deposito_id
  values (v_venda_numero, v_total, p_desconto, v_forma_pag_id, p_pessoa_id, nullif(p_observacoes, ''), 'concluida', p_deposito_id) -- ALTERADO: + p_deposito_id
  ...
  if v_pago_total > 0 then
    insert into lancamentos (descricao, valor, tipo, data_competencia, data_vencimento, status, data_pagamento, updated_at, venda_id) -- ALTERADO: + venda_id
    values ('Venda #' || v_venda_numero, v_pago_total, 'receber', v_today, v_today, 'pago', v_today, v_now, v_venda_id); -- ALTERADO: + v_venda_id
  end if;
  ...
  if v_fiado_total > 0 then
    ...
    insert into lancamentos (descricao, valor, tipo, data_competencia, data_vencimento, status, pessoa_nome, updated_at, venda_id) -- ALTERADO: + venda_id
    values ('A Receber — Fiado #' || v_venda_numero, v_fiado_total, 'receber', v_today, v_today, 'pendente', v_pessoa_nome, v_now, v_venda_id); -- ALTERADO: + v_venda_id
  end if;
