-- ============================================================
-- 🔒 LOCK do crédito do cliente em finalizar_venda — fecha a CORRIDA.
--
-- O buraco: o bloco de crédito lia o saldo com um SELECT sum() SEM lock e só
-- depois inseria o 'uso'. Dois checkouts simultâneos do MESMO cliente usando o
-- crédito da loja (em PCs diferentes, produtos diferentes → sem compartilhar o
-- lock de estoque) liam ambos o mesmo saldo, ambos passavam na trava, e ambos
-- gastavam o crédito → saldo do cliente ficava NEGATIVO (crédito gasto em dobro).
--
-- Correção: um `perform 1 from pessoas where id = p_pessoa_id for update` no
-- COMEÇO do bloco de crédito. O 2º checkout fica bloqueado até o 1º commitar;
-- aí ele lê o saldo JÁ debitado e recusa se não cobrir. Mesmo padrão do
-- `for update` que serializa a devolução e o estoque.
--
-- Corpo byte-a-byte idêntico à 2026-07-14_trava_pagamentos_venda.sql, exceto
-- a única linha do lock (marcada abaixo). Nome ordena depois de 07-14 (é a
-- última redefinição de finalizar_venda). Idempotente (create or replace).
-- ============================================================
