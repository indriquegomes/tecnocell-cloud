-- ============================================================================
-- movimentar_estoque: rejeita quantidade negativa (19/09)
--
-- Bug achado na revisão: quantidade negativa invertia a operação.
--   'saida' -5 virava v_ant - (-5) = v_ant + 5 (estoque SUBIA numa saída);
--   'ajuste' negativa gravava estoque negativo;
--   'perda'/'uso_interno' negativos geravam despesa 'pagar' NEGATIVA.
-- A action TS já faz Math.max(0, ...), mas o RPC é chamável direto (security definer),
-- então a trava precisa existir aqui também. Serializado ignora p_quantidade
-- (a quantidade vem da lista de IMEIs), por isso a guarda só vale pro não-serializado.
-- ============================================================================
