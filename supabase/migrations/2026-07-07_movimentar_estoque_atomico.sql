-- ============================================================
-- Movimentação de estoque atômica (conselho, bugs médios).
--
-- Antes: registrarMovimento/registrarMovimentos faziam read-modify-write no
-- estoque sem lock (race em concorrência) e, para produto serializado, saída/
-- ajuste baixavam o estoque sem tocar em numeros_serie (dessincronizava).
--
-- Agora: movimentar_estoque trava a linha (FOR UPDATE) e, para serializado:
--   - entrada exige IMEIs → insere numeros_serie (dedup) e soma os novos;
--   - saída exige IMEIs → marca 'defeito' e baixa o estoque pela contagem;
--   - ajuste é bloqueado (a contagem vem dos IMEIs).
-- Não-serializado segue entrada/saída/ajuste por quantidade.
-- Mantém a invariante estoque.quantidade == count(numeros_serie em_estoque).
--
