-- ============================================================
-- 🔴 CORREÇÃO DE ORDEM DE MIGRATION — restaura as DUAS travas da devolução.
--
-- Como isto aconteceu (bug de nomeação de arquivo, não de lógica):
--   A versão COMPLETA e correta de registrar_devolucao foi escrita por último,
--   mas salva com nome de data ANTERIOR (2026-07-15_devolucao_dinheiro_sai_do_caixa).
--   As versões PARCIAIS (parcial-fiado, troca-não-volta) foram escritas antes,
--   mas salvas com nomes de data POSTERIOR (2026-08-07 / 2026-08-08).
--   Migrations aplicam por ORDEM DE NOME, então a boa entra primeiro e as parciais
--   sobrescrevem depois — a função final (08-08) ficou SEM:
--     1. a TRAVA DE EXCEDENTE (não devolver mais do que foi vendido / re-devolver
--        a mesma venda / corrida entre duas telas) — perdida em 08-07/08-08.
--     2. a exclusão do DINHEIRO do lançamento financeiro — o dinheiro voltou a
--        virar lançamento "a pagar/pago" ALÉM de sair da gaveta (movimento_caixa
--        na action), contando a saída em dobro nos registros.
--
-- Esta migration tem nome que ordena por ÚLTIMO (2026-08-09) e re-aplica a versão
-- DEFINITIVA: trava de excedente + dinheiro fora do lançamento + troca/defeito/avaria
-- não volta ao estoque + serializado só 'ok' volta + abate parcial de fiado.
-- Corpo idêntico ao 2026-07-15_devolucao_dinheiro_sai_do_caixa (a versão correta).
-- Idempotente (create or replace); seguro reaplicar.
-- ============================================================

-- tipo 'devolucao' permitido no caixa (idempotente — já deve existir)
