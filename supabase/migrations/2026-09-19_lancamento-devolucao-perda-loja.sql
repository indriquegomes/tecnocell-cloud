-- ============================================================================
-- Lançamentos de devolução/perda ganham loja_id (19/09)
-- Antes: reembolso de devolução e perda/uso interno nasciam SEM loja -> viravam
-- "Sem loja" no financeiro. Agora herdam a loja da venda (devolução) ou do
-- depósito (perda).
-- ============================================================================
  ...
  if v_reembolso > 0.005 then
    ...
    if p_reembolsos is not null and jsonb_array_length(p_reembolsos) > 0 then
      ...
      for v_r in select * from jsonb_array_elements(p_reembolsos) loop
        ...
        if v_tipo_r = 'credito_conta' then
          ...
          insert into lancamentos (descricao, valor, tipo, data_competencia, data_vencimento, status, data_pagamento, forma_pagamento, pessoa_nome, conta_id, loja_id, updated_at)
          values ('Devolução — ' || coalesce(p_pessoa_nome, 'Cliente'), v_valor_r, 'pagar', v_today, v_today, 'pago', v_today, v_tipo_r, p_pessoa_nome, v_conta_r, v_loja_id, v_now);
        end if;
      end loop;
    ...
    -- ═══ MODO ANTIGO: uma forma só (segue idêntico) ═══
      ...
      if p_tipo_credito = 'credito_conta' then
        ...
        insert into lancamentos (descricao, valor, tipo, data_competencia, data_vencimento, status, data_pagamento, forma_pagamento, pessoa_nome, loja_id, updated_at)
        values ('Devolução — ' || coalesce(p_pessoa_nome, 'Cliente'), v_reembolso, 'pagar', v_today, v_today, 'pago', v_today, p_tipo_credito, p_pessoa_nome, v_loja_id, v_now);
      end if;
    end if;
  end if;
...
begin
  ...
  select loja_id into v_loja from depositos where id = p_deposito_id;

  -- TRAVA (19/09): quantidade negativa não pode inverter a operação.
  if not v_controla and p_quantidade < 0 then
  ...
  if p_operacao in ('perda', 'uso_interno') then
    insert into lancamentos (descricao, valor, tipo, status, valor_pago, categoria, data_competencia, data_vencimento, data_pagamento, created_at, updated_at, loja_id)
    values (
      ...
      v_now, v_now, v_now, v_now, v_now, v_loja
    );
  end if;
