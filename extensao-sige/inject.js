// Roda DENTRO da página (não no mundo isolado da extensão) porque só assim dá pra
// trocar o fetch/XHR que o próprio SIGE usa. É daqui que sai o dado valioso: o corpo
// exato que o SIGE manda pra API quando a Mariana finaliza uma venda.
(() => {
  const MAX = 20000 // corpo maior que isso vira resumo — não vale trafegar planilha inteira

  // Credencial NUNCA sai da máquina. O token do SIGE dá acesso total à conta;
  // registrar isso num banco seria trocar um problema por outro bem pior.
  const limpa = (obj) => {
  ...
  window.fetch = async function (...args) {
    ...
    // Ler a resposta consome o stream — por isso o clone. Sem ele, o SIGE quebra.
  ...
  // ---- XMLHttpRequest (o SIGE ainda usa em partes antigas)
