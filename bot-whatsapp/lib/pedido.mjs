...
function formataTelefone(tel) {
  ...
  if (d.startsWith('55') && d.length >= 12) return '+55 (' + d.slice(2, 4) + ') ' + d.slice(4, 9) + '-' + d.slice(9)
  if (d.length >= 10) return '(' + d.slice(0, 2) + ') ' + d.slice(2, 7) + '-' + d.slice(7)
  return d || '(número não identificado)'
}
...
// encaminha pra vendedora como antes.
export async function respondePedido(sock, slug, jid, telefone, contexto) {
