...
import { conferirImportacaoPessoas, aplicarImportacaoPessoas, type ConferenciaPessoas } from './actions'

export function ImportarPessoas() {
  const [erros, setErros] = useState<string[]>([])
  const [conf, setConf] = useState<ConferenciaPessoas | null>(null)
  ...
  function conferir(file: File, aba?: string) {
    ...
    startTransition(async () => {
      const res = await conferirImportacaoPessoas(fd)
  ...
  function onAplicar() {
    ...
    startTransition(async () => {
      const res = await aplicarImportacaoPessoas(conf.mudancas)
      ...
      {sucesso && (
        <div className="rounded-xl border border-green-200 bg-green-50 p-4">
          <p className="text-sm font-semibold text-green-700">
            Pronto! {sucesso.novos} cadastro(s) novo(s) e {sucesso.atualizados} atualizado(s).
          </p>
        </div>
      )}
      ...
      {conf && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              { rot: 'Linhas no arquivo', v: conf.linhas, cor: 'text-gray-900' },
              { rot: 'Cadastros novos', v: conf.novos, cor: 'text-emerald-600' },
          ...
          {conf.mudancas.length > 0 ? (
            <>
              ...
              <div className="flex gap-2">
                <button onClick={onAplicar} disabled={pending}
                  ...
                  {pending ? 'Gravando...' : `Confirmar e gravar ${conf.mudancas.length} cadastro(s)`}
                </button>
            ...
            <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 text-sm text-gray-600">
              Nada a fazer: todos os {conf.semMudanca} cadastro(s) deste arquivo ja estao iguais aqui.
            </div>
          )}
        </div>
      )}
