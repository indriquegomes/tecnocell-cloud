...
// Formas que fazem sentido para movimentos de caixa físico
...
export function OperacaoClient({
  ...
  const [reforcoState, reforcoAction, reforcoPending] = useActionState<ActionState, FormData>(
    registrarReforco,
    null,
  )
  const [retiradaState, retiradaAction, retiradaPending] = useActionState<ActionState, FormData>(
    registrarRetirada,
    null,
  )
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Status do caixa */}
      <div
        className={`rounded-2xl border p-5 ${caixaAberto ? 'border-green-200 bg-green-50' : 'border-gray-200 bg-white'}`}
      >
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-gray-500">Status do Caixa</p>
            <p
              className={`text-xl font-bold mt-0.5 ${caixaAberto ? 'text-green-700' : 'text-gray-500'}`}
            >
          ...
          <div
            className={`h-3 w-3 rounded-full ${caixaAberto ? 'bg-green-500' : 'bg-gray-300'}`}
          />
        </div>
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            <button
              ...
              className={`rounded-2xl border p-4 text-left transition hover:shadow-md ${panel === 'fechar' ? 'border-blue-400 bg-blue-50' : 'border-blue-200 bg-white hover:border-blue-300'}`}
            >
            ...
            <button
              ...
              className={`rounded-2xl border p-4 text-left transition hover:shadow-md ${panel === 'reforco' ? 'border-green-400 bg-green-50' : 'border-green-200 bg-white hover:border-green-300'}`}
            >
              <div className="mb-2 text-2xl">💰</div>
            ...
            <button
              ...
              className={`rounded-2xl border p-4 text-left transition hover:shadow-md ${panel === 'retirada' ? 'border-red-400 bg-red-50' : 'border-red-200 bg-white hover:border-red-300'}`}
            >
              <div className="mb-2 text-2xl">💸</div>
            ...
            <button
              ...
              className={`rounded-2xl border p-4 text-left transition hover:shadow-md ${panel === 'saldo' ? 'border-cyan-400 bg-cyan-50' : 'border-cyan-200 bg-white hover:border-cyan-300'}`}
            >
              <div className="mb-2 text-2xl">📊</div>
          ...
          {panel === 'reforco' && (
            <div className="rounded-2xl border border-green-200 bg-white p-6 shadow-sm space-y-4">
              ...
              <form action={reforcoAction} className="flex flex-wrap gap-4 items-end">
                ...
                >
                  {reforcoPending ? 'Salvando...' : 'Registrar Reforço'}
                </button>
              </form>
              <FeedbackMsg state={reforcoState} />
          ...
          {panel === 'retirada' && (
            <div className="rounded-2xl border border-red-200 bg-white p-6 shadow-sm space-y-4">
              ...
              <form action={retiradaAction} className="flex flex-wrap gap-4 items-end">
                <input type="hidden" name="caixa_id" value={caixaAberto.id} />
                <div className="flex-1 min-w-40">
                  <label className="mb-1.5 block text-sm font-medium text-gray-700">Forma de Pagamento</label>
                  <select name="forma_pagamento" className="field">
                ...
                >
                  {retiradaPending ? 'Salvando...' : 'Registrar Retirada'}
                </button>
              </form>
              <FeedbackMsg state={retiradaState} />
