...
import { receberNota, estornarNota } from '../actions'
...
export default async function NotaEntradaDetalhe({
  ...
  const depMap = new Map((depositos ?? []).map((d) => [d.id, d.nome]))
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Adicionar item — com busca de produto */}
      {nota.status === 'pendente' && (
        <AddItemNota notaId={id} produtos={produtos ?? []} depositos={depositos ?? []} />
      )}
            ...
            {(itens ?? []).length === 0 ? (
              ...
              return (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-500">{depMap.get(item.deposito_id) ?? '—'}</td>
